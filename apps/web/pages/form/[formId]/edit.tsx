...
const schema = z.object({
  ...
  updateToken: z.string().min(10, { message: 'Update token must be at least 10 characters' }),
...
const EditFormPage: NextPage<Props> = ({ session, form }) => {
  ...
  return (
    <MainContainer>
      ...
      <form className="flex flex-col gap-3 max-w-fit" onSubmit={handleSubmit(submitHandler)}>
        <div className="form-control">
          <label className="input-group">
            <span>Name</span>
            <input type="text" className="input input-bordered" placeholder="My Special Form" {...register("name")} />
          </label>
        ...
        <div className="form-control">
          <label className="input-group">
            ...
            <input type="text" className="input input-bordered" placeholder="Update Token" {...register("updateToken")} />
          </label>
        </div>
        {errors.updateToken && <p className="text-sm text-red-500">{errors.updateToken?.message}</p>}
        <div className="form-control">
          <label className="input-group">
            <span>Submissions Paused</span>
            <input type="checkbox" className="input input-bordered checkbox" placeholder="Submissions Paused"{...register("submissionsPaused")} />
          </label>
        </div>
